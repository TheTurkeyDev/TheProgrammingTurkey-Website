        ...
        <AuthPageWrapper history={props.history} perm="projects.editstatus">
            <div className="fluid-container text-center mt-4" style={{ height: "80vh" }}>
                ...
                <div className="row m-0 mt-2">
                    <div className="dropdown col ml-auto">
                    </div>
                    <div className="dropdown col-auto">
                        <button className="dropbtn" id="project_button" onClick={() => showProjects()} >{selectedProject ? selectedProject : "Project"}</button>
                        <div id="projects" className="dropdown-menu" style={{ display: (projectsVisible ? "block" : "none") }}>
                            <input className="dropdown-item" type="text" placeholder="Search.." value={filter} onChange={(e) => setFilter(e.target.value)} />
                            {
                                Object.keys(projects).sort((a, b) => a.localeCompare(b))
                                    ...
                                    .map(proj => {
                                        return (<span key={proj} className="dropdown-item" onClick={() => { setProject(proj) }}>{proj}</span>);
                                    })
                            }
                        </div>
                    </div>
                    <select className="col-auto mr-2" value={selectedVersion} onChange={e => { setSelectedVersion(e.target.value) }}>
                    ...
                    <select className="col-auto mr-2" value={selectedStatus} onChange={e => setSelectedStatus(e.target.value)}>
                    ...
                    <button className="col-auto mr-2" onClick={() => updateProject()}>
                        Update
                    </button>
